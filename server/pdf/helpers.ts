...
// ─── 定時 calculation: append net work hours to each shift ──────────
// "A勤務：7:00～15:30" + break 45min → "A勤務：7:00～15:30 (7.75)"

/** Extract break minutes per shift name from breakTimeDay text.
 *  Looks for "合計XXX分" per line, or "(XX分)" if no 合計. */
function parseBreakMinutes(breakText: string): Map<string, number> {
  ...
  for (const line of breakText.split("\n")) {
    // Extract shift name: "A勤務" or "昼勤" etc.
    const nameMatch = line.match(/^([A-Za-zＡ-Ｚ\u4e00-\u9fff]+勤?務?勤?)[\s：:]/);
    if (!nameMatch) continue;
    const name = nameMatch[1];
    // Prefer "合計XX分"
    const totalMatch = line.match(/合計(\d+)分/);
    if (totalMatch) { map.set(name, Number(totalMatch[1])); continue; }
    // Fallback: sum all "(XX分)" in the line
    let sum = 0;
    for (const m of line.matchAll(/[（(](\d+)分[）)]/g)) sum += Number(m[1]);
    if (sum > 0) map.set(name, sum);
  }
  return map;
}

/** Calculate 定時 (net work hours) for each shift line.
 *  Appends "(Xh)" for standard shifts, "(Xh)+残業(Yh)" for overtime shifts.
 *  Base 定時 = most common net hours across all shifts (mode). */
export function appendTeiji(workHours: string, breakTimeDay: string | null | undefined): string {
  ...
  // For simple formats: if breakMap is empty, try parsing a single (XX分) value
  ...
  // Step 1: Calculate net minutes for each shift
  const shiftData: { line: string; netMins: number }[] = [];
  for (const line of lines) {
    const name = extractShiftName(line);
    const breakMins = name ? (breakMap.get(name) ?? 0) : 0;
    const netMins = calcNetMinutes(line, breakMins);
    shiftData.push({ line, netMins });
  }

  // Step 2: Find 定時 base = most common net hours (mode)
  const validMins = shiftData.filter(s => s.netMins > 0).map(s => s.netMins);
  const baseMins = findMode(validMins) ?? 0;

  // Step 3: Append teiji + overtime labels
...
/** Extract shift name from line: "A勤務：..." → "A勤務", "昼勤：..." → "昼勤" */
function extractShiftName(line: string): string | null {
  const m = line.match(/^([A-Za-zＡ-Ｚ\u4e00-\u9fff]+勤?務?勤?)[\s：:]/);
  return m ? m[1] : null;
}

/** Calculate net work minutes for a shift line (duration - break). */
...
/** Format minutes as decimal hours: 465 → "7.75", 480 → "8.0" */
