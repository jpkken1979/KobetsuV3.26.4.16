...
export function parseKoritsuPdfText(text: string): KoritsuParsedResult {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // ── COMPOUND: 品質課/工務課 + 係 + supervisor on same line ──
    // Pattern: "品 質 課 古川 市朗 品質係（州の崎） 渡部 浩司 係長 0569-20-0332"
    const compoundKakariMatch = line.match(
      /^(品\s*質\s*課|工\s*務\s*課)\s+(.+?)\s+((?:品質|工務)係(?:[（(].+?[）)])?)\s+(.+?)\s+(係長)(?:\s+(\d{4}-\d{2}-\d{4}))?$/,
    );
    if (compoundKakariMatch) {
      const deptType = compoundKakariMatch[1].replace(/\s+/g, "") as "品質課" | "工務課";
    ...
    if (supportTwoNamesMatch) {
      const deptType = supportTwoNamesMatch[1].replace(/\s+/g, "") as "品質課" | "工務課";
    ...
    const kakariMatch = line.match(
      /^((?:品質|工務|経営企画|経理|営業)係(?:[（(].+?[）)])?)\s+(.+?)\s+(係長)(?:\s+(\d{4}-\d{2}-\d{4}))?$/,
    );
    if (kakariMatch) {
      ...
      if (kakariName.startsWith("品質") || kakariName.startsWith("工務")) {
        allKakari.push({ lineIdx: i, kakariName, supervisorName, phone });
      }
      pendingPhone = null;
      continue;
    ...
    if (supportDeptMatch && !line.includes("係")) {
      const deptType = supportDeptMatch[1].replace(/\s+/g, "") as "品質課" | "工務課";
